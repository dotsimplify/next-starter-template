...
export async function generateMetadata(): Promise<Metadata> {
  const slug = "chart-2024";
  const canonical = `${process.env.NEXT_PUBLIC_CLIENT_URL}/satta-king-2024`;
  const str = `satta king chart 2024`;
  const ogImage = `${
...
async function getHeaderText(): Promise<MetaData> {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_LIVE_API_URL}/get-meta-by-page/chart-2024`,
    {
    ...
    <main>
      ...
      <div className="grid items-center grid-cols-1 gap-4 px-4 py-4 md:grid-cols-3 ">
        {games &&
          ...
          games.map((one, index) => {
            return (
              <Link key={index} href={`/satta-king-2024/${one.slug}`}>
                <Pill
